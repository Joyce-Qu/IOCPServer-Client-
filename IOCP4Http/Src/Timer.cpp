...
unsigned HbTimer::checkHeartbeatWorker(LPVOID arg)
{
	...
	liDueTime.QuadPart = -10000000LL * 1;   //1̑
    ...
    while (1)
    {
        ...
        LockGuard lk(&pIocpServer->m_csClientList);
        {
            for (auto it = pConnectedClientList->begin();
				it != pConnectedClientList->end(); ++it)
            {
                ...
                LockGuard lk(&pClientCtx->m_csLock);
                {
                    ...
                    {
                        //postClose
                        if (FALSE == PostQueuedCompletionStatus(pIocpServer->m_hComPort,
							0, (ULONG_PTR)pClientCtx, NULL))
                        {
